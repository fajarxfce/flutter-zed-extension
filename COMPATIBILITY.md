...
| Zed manifest, Dart LSP foundation, and Flutter snippets | **Supported, live validation unverified** | The repository registers metadata, but no local Zed CLI/application was available to live-load it. Do not infer live language-server/debug behavior from fixture tests. | `README.md`; `.sisyphus/evidence/task-17-fresh-project.txt` |
...
`setup_project` owns only this workflow's stable task/debug labels. It parses both existing `.zed/tasks.json` and `.zed/debug.json` before writing either, preserves unrelated entries and top-level task settings, writes atomically with mode `0600`, and is idempotent after a successful setup. Take a normal project backup/review the `dry_run=True` diff before upgrading generated configuration; do not hand-edit entries owned by this workflow if you expect regeneration to replace them. The fixture workflow generated both files, while boundary tests reject unsafe inputs without creating `.zed`; see `.sisyphus/evidence/task-16-full-workflow.txt` and `.sisyphus/evidence/task-16-boundary-failures.txt`.
